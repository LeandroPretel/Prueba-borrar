...
$companyId = firstCompanyId(); //Obtención del id de la primera empresa disponible (función únicamente válida para ejemplos)
    ...
    $budgets = App\Anfix\CustomerBudget::all($companyId); //Obtenemos el presupuesto con el id indicado o un error si no existe
    ...
    $budget = App\Anfix\CustomerBudget::first([],$companyId); //Obtenemos el presupuesto con el id indicado o un error si no existe

    //Reporte del presupuesto
    $report = \App\Anfix\DocumentReport::customerBudget([
    ...
    $mydoc = \App\Anfix\MyDocuments::send([
    ...
    \App\Anfix\MyDocuments::download($report->MyDocuments->EntryId,'../download/budget.pdf');
